...
通所介護施設において、利用者の送迎表を作成する作業が、利用者数が増えるほどに煩雑化していきます。結果業務を圧迫したり、残業の原因になることもあります。
介護ソフトと連携した配車管理ソフトもありますがコストなどの面から、地方や小規模な施設、設備投資に積極的でない施設などでは導入がされていないのが現状です。そんな状況を少しでも改善できればと思い、簡易的ではありますが作成しました。
...
_ゲストログインをした場合には上記データは全て登録してあります。_
...
既に選択されているドライバーを選択した場合、警告が出ます。  
問題がある場合には変更してください。

### 手順３（省略可）

反転挿入ボタンを押すことで、「迎え」に登録したコースを逆順にして、「送り」に登録することができます。登録後も編集可能です。

### 手順４

全て入力したら、「一時保存」または「保存」ボタンを押し、保存してください。  
